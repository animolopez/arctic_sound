...
for l in mos_segdur:
    for m in block:
        if (l * m) <= max_rand_segdur:
            ...
            #mos_seginfo = divmod(numsamp, mos_segsamp)
            #mos_segrep = mos_seginfo[0] + 1
            ...
            for o in np.arange(0,rand_segrep,1):
                ...
                if o == 0:
                    ...
                    order_array = np.concatenate((order_array, order), 0)
            order_matrix = order_array.reshape(rand_segrep, m)
            ...
            for n in np.arange(0,(cutoff.size-1),1):
                ...
                for o in np.arange(0,rand_segrep,1):
                    #order = np.random.permutation(m)
                    #spec_rand_seg_pow = rand_seg_speech_pow[o][[order],:]
                    ...
                    for p in np.arange(0,m,1):
                        seg_pow_mean_array = np.full(mos_segsamp, fwd_seg_pow_mean[order_matrix[o,p]])
