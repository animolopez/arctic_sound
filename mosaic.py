...
segdur = 40 # Segment duration [ms] (segdur > 20 [ms])
output_filename = 'NJ01F001_44100_mos_%dbands_%dms.wav' % ((cutoff.size - 1), segdur) # Do not add a extension (.wav)
...
# Zero padding
segsamp = int(np.round(segdur * 0.001 * fs))
...
#y = np.zeros(numsamp)
env = sqrtCosEnv(segdur*0.001,rft=0.005,fs=fs)
